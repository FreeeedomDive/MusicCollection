...
fun FileSystemNodeDto.convertToModel(parent: NodeModel? = null) : NodeModel{
    return NodeModel(this.id, parent, null, this.path, if (this.type == NodeTypeDto.File) NodeType.File else NodeType.Directory, this.directoryData, this.tags)
}

fun FileSystemRootDto.convertToModel() : NodeModel{
    return NodeModel(this.id, null, this.name, this.path, NodeType.Root, null, null)
}